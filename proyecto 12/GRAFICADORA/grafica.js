...
function Range(min, max, inc) {// 0 10 1 [0,1,2,3,4,5,6,...,2]
    let result = [];
...
function Plot(xMin, xMax, inc, fx, valueTable) {
    ...
    let xScale = (width - 1) / xRange;
    let Ys = Xs.map(fx);
    if (fx(0) === Infinity) {
        Xs = Xs.filter(x => x !== 0);
    }
    Ys = Ys.filter(y => y !== Infinity);

    let yMin = Math.min(...Ys);
    let yMax = Math.max(...Ys);
    let yRange = Math.abs(yMin - yMax) + 1;
    let yScale = (height - 1) / yRange;
    ...
    for (let i = 0; i < (Xs.length); i++) {
        ...
        valueY = valueY.toFixed(1); //Un solo decimal
        let StrvalueY = valueY.toString();
        let posX = Math.round(Xs[i] * xScale + yAxis + (drawXAxis + 1) * 81);
    ...
    for (let i = 0; i < Xs.length; i++) {
        ...
        let posY = Math.round((height - 1) - (Ys[i] * yScale + xAxis));
